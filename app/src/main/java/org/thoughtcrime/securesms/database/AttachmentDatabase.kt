...
class AttachmentDatabase(
    ...
    fun deleteAttachmentsForMessages(messageIds: Array<String?>) {
        val idsAsString = StringBuilder().run {
            messageIds.indices.forEach { i ->
                if (i != 0) append(" OR ")
                append("$MMS_ID = ").append(messageIds[i])
            }
        }.toString()
    ...
    private fun deleteAttachmentsOnDisk(mmsAttachmentInfos: Sequence<MmsAttachmentInfo>) {
        mmsAttachmentInfos
        ...
        if (mmsAttachmentInfos.any { it.isImage() || it.isThumbnailNonNull() }) {
    ...
    @Throws(MmsException::class)
    private fun setAttachmentData(destination: File, inputStream: InputStream): DataInfo {
        try {
    ...
    internal inner class ThumbnailFetchCallable(private val attachmentId: AttachmentId) :
        ...
        override fun call(): InputStream? {
            ...
            getDataStream(attachmentId, THUMBNAIL, 0)?.let { return it }

            if (
                getAttachment(attachmentId)
                    ?.takeIf(DatabaseAttachment::hasData)
                    ?.contentType
                    ?.let(MediaUtil::isVideoType) != true
            ) return null

            val data = generateVideoThumbnail(attachmentId) ?: return null
        ...
        private fun generateVideoThumbnail(attachmentId: AttachmentId): ThumbnailData? {
            val dataInfo = getAttachmentDataFileInfo(attachmentId, DATA) ?: run {
            ...
            val bitmap = MediaMetadataRetriever().apply { setDataSource(dataSource) }
                .getFrameAtTime(1000)
